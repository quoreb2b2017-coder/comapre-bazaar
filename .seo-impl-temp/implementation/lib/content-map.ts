/**
 * lib/content-map.ts — single source of truth for the hub-and-spoke
 * internal-linking system. RelatedContent, breadcrumbs, and the sitemap
 * all read from this.
 *
 * Rule of the system:
 *   post.hub        → the ONE comparison hub a post funnels equity into
 *   hub.posts       → derived automatically (never hand-curate twice)
 */
...
export type Hub = {
  ...
  whitepapers?: string[]; // whitepaper slugs
};

export type Post = {
  slug: string;
  title: string;
  hub: string | null; // hub slug — the post's primary internal-link target
  intent: "informational" | "commercial";
  lastVerified: string; // YYYY-MM-DD, drives dateModified + on-page stamp
};
...
// ── Derived lookups (the interlinking engine) ─────────────────────────────
...
/** Posts supporting a given hub → feeds the hub's "Further reading" module */
export function postsForHub(hubSlug: string, limit = 5): Post[] {
  return posts.filter((p) => p.hub === hubSlug).slice(0, limit);
}

/** Sibling posts (same hub) → feeds a post's "Related guides" module */
...
/** Hub a post should funnel into → the post's primary internal link / CTA */
export function primaryHubForPost(postSlug: string): Hub | null {
  const post = posts.find((p) => p.slug === postSlug);
  return post?.hub ? hubBySlug[post.hub] ?? null : null;
}

/** Build-time gap report: hubs with <3 supporting posts = content-calendar backlog */
