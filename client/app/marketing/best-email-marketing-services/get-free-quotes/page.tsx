...
const EmailMarketingGetQuotesForm = () => {
  ...
  useEffect(() => {
    document.title = "Get Email Marketing Quotes | Compare-Bazaar";
  }, []);
  ...
  const validateForm = () => {
    ...
    if (!formData.currentEmailService) {
      newErrors.currentEmailService = 'Please complete this required field.';
    }
    if (!formData.subscriberCount) {
      newErrors.subscriberCount = 'Please complete this required field.';
    }
    if (!formData.monthlyCampaigns) {
      newErrors.monthlyCampaigns = 'Please complete this required field.';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        ...
        current_email_service: formData.currentEmailService,
        subscriber_count: formData.subscriberCount,
        monthly_campaigns: formData.monthlyCampaigns,
  ...
  const importantFeatures = [
    { id: 'automation', label: 'Email Automation & Workflows', icon: <Zap className="w-5 h-5" /> },
    { id: 'templates', label: 'Email Templates & Design Tools', icon: <Palette className="w-5 h-5" /> },
    { id: 'analytics', label: 'Advanced Analytics & Reporting', icon: <BarChart3 className="w-5 h-5" /> },
    { id: 'segmentation', label: 'Audience Segmentation', icon: <Users className="w-5 h-5" /> },
    { id: 'a-b-testing', label: 'A/B Testing Tools', icon: <BarChart className="w-5 h-5" /> },
    { id: 'deliverability', label: 'High Deliverability Rates', icon: <Send className="w-5 h-5" /> }
  ];
              ...
              <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 lg:p-10 border border-gray-100 backdrop-blur-sm transform transition-all duration-300 hover:shadow-3xl w-full flex flex-col h-full">
                ...
                <div className="mb-6 pb-4 border-b border-gray-200">
                  ...
                  <p className="text-base text-gray-600 leading-relaxed">
                    Connect with top email marketing providers. Compare features, pricing, and find the ideal email marketing solution to grow your business and engage your audience.
                  </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5 flex-1 flex flex-col">
                  ...
                  <div className="group">
                    <label htmlFor="currentEmailService" className="block text-sm font-bold text-gray-800 mb-2 transition-colors group-focus-within:text-[#ff8633]">
                      Currently Using Email Marketing Service? <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <select
                        id="currentEmailService"
                        name="currentEmailService"
                        value={formData.currentEmailService}
                        onChange={(e) => handleSelectChange('currentEmailService', e.target.value)}
                        onFocus={() => setFocusedField('currentEmailService')}
                        onBlur={() => setFocusedField(null)}
                        className={`w-full px-4 py-3.5 pr-12 border-2 rounded-xl text-gray-900 bg-white 
                          transition-all duration-300 ease-in-out cursor-pointer
                          focus:outline-none focus:ring-4 focus:ring-[#ff8633]/20 focus:border-[#ff8633] 
                          ...
                          ${focusedField === 'currentEmailService' ? 'shadow-lg scale-[1.01]' : ''}
                        `}
                      >
                        <option value="">Select option</option>
                        <option value="Yes - Looking to switch">Yes - Looking to switch</option>
                        <option value="Yes - Looking for better features">Yes - Looking for better features</option>
                        <option value="No - First time user">No - First time user</option>
                      </select>
                      <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                        <ChevronDown className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${focusedField === 'currentEmailService' ? 'text-[#ff8633] rotate-180' : ''}`} />
                      </div>
                    </div>
                    {errors.currentEmailService && (
                      <p className="mt-1.5 text-sm text-red-600 font-medium animate-slideDown flex items-center">
                        <span className="mr-1">⚠</span> {errors.currentEmailService}
                      </p>
                    )}
                  </div>

                  {/* Subscriber Count and Monthly Campaigns */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="group">
                      <label htmlFor="subscriberCount" className="block text-sm font-bold text-gray-800 mb-2 transition-colors group-focus-within:text-[#ff8633]">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => handleSelectChange('subscriberCount', e.target.value)}
                          onFocus={() => setFocusedField('subscriberCount')}
                          onBlur={() => setFocusedField(null)}
                          className={`w-full px-4 py-3.5 pr-12 border-2 rounded-xl text-gray-900 bg-white 
                            transition-all duration-300 ease-in-out cursor-pointer
                            focus:outline-none focus:ring-4 focus:ring-[#ff8633]/20 focus:border-[#ff8633] 
                            ...
                            ${focusedField === 'subscriberCount' ? 'shadow-lg scale-[1.01]' : ''}
                          `}
                        >
                        ...
                        <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                          <ChevronDown className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${focusedField === 'subscriberCount' ? 'text-[#ff8633] rotate-180' : ''}`} />
                        </div>
                      </div>
                      {errors.subscriberCount && (
                        <p className="mt-1.5 text-sm text-red-600 font-medium animate-slideDown flex items-center">
                          <span className="mr-1">⚠</span> {errors.subscriberCount}
                        </p>
                      )}
                    </div>

                    <div className="group">
                      <label htmlFor="monthlyCampaigns" className="block text-sm font-bold text-gray-800 mb-2 transition-colors group-focus-within:text-[#ff8633]">
                        Monthly Campaigns <span className="text-red-500">*</span>
                      </label>
                      <div className="relative">
                        <select
                          id="monthlyCampaigns"
                          ...
                          onChange={(e) => handleSelectChange('monthlyCampaigns', e.target.value)}
                          onFocus={() => setFocusedField('monthlyCampaigns')}
                          onBlur={() => setFocusedField(null)}
                          className={`w-full px-4 py-3.5 pr-12 border-2 rounded-xl text-gray-900 bg-white 
                            transition-all duration-300 ease-in-out cursor-pointer
                            focus:outline-none focus:ring-4 focus:ring-[#ff8633]/20 focus:border-[#ff8633] 
                            hover:border-gray-400 hover:shadow-md
                            appearance-none
                            ${errors.monthlyCampaigns 
                              ? 'border-red-500 bg-red-50 focus:ring-red-200 focus:border-red-500' 
                              : 'border-gray-300'
                            }
                            ${focusedField === 'monthlyCampaigns' ? 'shadow-lg scale-[1.01]' : ''}
                          `}
                        >
                          <option value="">Select frequency</option>
                          <option value="1 - 4">1 - 4 campaigns/month</option>
                          <option value="5 - 10">5 - 10 campaigns/month</option>
                          <option value="11 - 20">11 - 20 campaigns/month</option>
                          <option value="20+">20+ campaigns/month</option>
                        </select>
                        <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                          <ChevronDown className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${focusedField === 'monthlyCampaigns' ? 'text-[#ff8633] rotate-180' : ''}`} />
                        </div>
                      </div>
                      {errors.monthlyCampaigns && (
                        <p className="mt-1.5 text-sm text-red-600 font-medium animate-slideDown flex items-center">
                          <span className="mr-1">⚠</span> {errors.monthlyCampaigns}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-4 bg-gradient-to-r from-orange-50 to-blue-50 rounded-xl border border-orange-200">
                    ...
                    <label htmlFor="emailUpdates" className="text-sm text-gray-700 cursor-pointer">
                      I'd like to receive email updates about email marketing solutions and best practices.
                    </label>
                  </div>
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <div className="mb-6">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold mb-3 leading-tight">
                      Grow Your Business with Powerful Email Marketing
                    </h2>
                    <p className="text-blue-200 text-sm leading-relaxed">
                      Discover the best email marketing platforms that help you reach your audience, build relationships, and drive conversions.
                    </p>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-[#ff8633]/20 to-orange-600/20 rounded-xl p-4 mb-5 border border-[#ff8633]/30">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm text-blue-100">Compare 30+ top email marketing providers</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-[#ff8633]"></div>
                        <p className="text-sm text-blue-100">Get personalized recommendations based on your subscriber count</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-[#ff8633]"></div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-[#ff8633]"></div>
                        <p className="text-sm text-blue-100">Expert guidance from email marketing specialists</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-5">
                    <div className="bg-white/15 backdrop-blur-md rounded-xl p-3 border border-white/20 hover:bg-white/20 transition-all duration-300">
                      ...
                      <div className="text-xs text-blue-200">Businesses Served</div>
                    </div>
                    <div className="bg-white/15 backdrop-blur-md rounded-xl p-3 border border-white/20 hover:bg-white/20 transition-all duration-300">
                    ...
                    <div className="bg-white/15 backdrop-blur-md rounded-xl p-3 border border-white/20 hover:bg-white/20 transition-all duration-300">
                      ...
                      <div className="text-xs text-blue-200">Email Platforms</div>
                    </div>
                    <div className="bg-white/15 backdrop-blur-md rounded-xl p-3 border border-white/20 hover:bg-white/20 transition-all duration-300">
                  ...
                  {/* Top Email Marketing Features */}
                  <div className="mb-5">
                    <h3 className="text-lg font-bold mb-3 text-white">Top Email Marketing Features</h3>
                    <div className="space-y-2">
                      {[
                        { icon: <Zap className="w-5 h-5" />, text: 'Email Automation', benefit: 'Save 15+ hours weekly', color: 'from-yellow-400 to-orange-500' },
                        { icon: <Palette className="w-5 h-5" />, text: 'Beautiful Templates', benefit: '1000+ ready-made designs', color: 'from-pink-400 to-rose-500' },
                        { icon: <BarChart3 className="w-5 h-5" />, text: 'Advanced Analytics', benefit: 'Track every click & open', color: 'from-blue-400 to-cyan-500' },
                        { icon: <Users className="w-5 h-5" />, text: 'Smart Segmentation', benefit: 'Target the right audience', color: 'from-purple-400 to-indigo-500' },
                        { icon: <Eye className="w-5 h-5" />, text: 'A/B Testing', benefit: 'Optimize open rates 40%', color: 'from-green-400 to-emerald-500' },
                        { icon: <Send className="w-5 h-5" />, text: 'High Deliverability', benefit: '98% inbox delivery rate', color: 'from-red-400 to-pink-500' }
                      ].map((feature, index) => (
                        <div 
                          key={index}
                    ...
                    <div className="space-y-2.5">
                      <div className="flex items-start gap-3 p-3 bg-gradient-to-r from-[#ff8633]/20 to-orange-600/20 rounded-lg border border-[#ff8633]/30">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-blue-200">Fill out the form with your business details and email marketing needs</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-gradient-to-r from-blue-600/20 to-blue-500/20 rounded-lg border border-blue-500/30">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-blue-200">Our system matches you with 3-5 perfect email marketing providers</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-gradient-to-r from-green-500/20 to-green-600/20 rounded-lg border border-green-500/30">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-blue-200">Review personalized quotes and select the best platform for your business</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="space-y-3">
                      <div className="bg-white/10 rounded-xl p-3 border border-white/15">
                        <p className="text-sm text-blue-100 italic mb-2">"Switched to a better email platform and saw our open rates jump from 18% to 32%! The comparison tool was incredibly helpful."</p>
                        <p className="text-sm text-[#ff8633] font-semibold">- Jennifer L., E-commerce Manager</p>
                      </div>
                      <div className="bg-white/10 rounded-xl p-3 border border-white/15">
                        <p className="text-sm text-blue-100 italic mb-2">"Found the perfect email marketing solution in just 1 day. Saved us $300/month and improved our ROI significantly."</p>
                        <p className="text-sm text-[#ff8633] font-semibold">- David R., Marketing Director</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-white/20">
                    ...
                    <div className="mt-3 bg-gradient-to-r from-[#ff8633]/20 to-orange-600/20 rounded-lg p-3 border border-[#ff8633]/30">
                      ...
                      <p className="text-xs text-blue-100">Get matched with 3-5 perfect email marketing providers within 24 hours</p>
                    </div>
                  </div>
                </div>
              </div>
...
export default EmailMarketingGetQuotesForm;
