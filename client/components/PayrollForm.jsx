...
import {
    Building2,
    EMPLOYEE_COUNT_ICONS,
    Mail,
    MapPin,
    PAYROLL_SOLUTION_ICONS,
    Phone,
    User,
    Users,
...
const PayrollForm = ({ onClose }) => {
    ...
    const payrollSolutions = [
        ...
        'Both payroll software and service',
    ...
    const renderStep = () => {
        switch (currentStep) {
            case 1:
                return (
                    <div>
                        <QuoteFormStepTitle title="What payroll solution are you looking for?" />
                        <QuoteFormOptionGrid cols={3}>
                            {payrollSolutions.map((option) => (
                                <QuoteFormRadioOption
                                    key={option}
                                    selected={formData.payrollSolution === option}
                                    onSelect={() => handleRadioChange('payrollSolution', option)}
                                    label={option}
                                    icon={PAYROLL_SOLUTION_ICONS[option] || Users}
                                />
                            ))}
                        </QuoteFormOptionGrid>
                    </div>
            ...
            case 2:
                return (
                    <div>
                        <QuoteFormStepTitle title="How many employees do you have?" />
                        <QuoteFormOptionGrid cols={3}>
                            {employeeCounts.map((option) => (
                                <QuoteFormRadioOption
                                    key={option}
                                    selected={formData.payrollEmployee === option}
                                    onSelect={() => handleRadioChange('payrollEmployee', option)}
                                    label={option}
