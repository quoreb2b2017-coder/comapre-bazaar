...
const NAV = [
  ...
  { to: '/excel-queue', icon: FileSpreadsheet, label: 'Excel Queue' },
...
const SUPER_ADMIN_NAV = [
  { to: '/super-admin/google-analytics', icon: Shield, label: 'Google Analytics' },
]
  ...
  return (
    ...
    >
      ...
      {isSuperAdmin ? (
        <>
          <p
            className={`px-4 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-widest text-white/35 ${collapsed ? 'sr-only' : ''}`}
          >
            Super Admin
          </p>
            ...
            {SUPER_ADMIN_NAV.map(({ to, icon: Icon, label }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `group flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-sm font-medium border border-transparent ${
                    isActive
                      ? 'bg-[#F58220]/20 text-white shadow-inner border-[#F58220]/30 ring-1 ring-[#F58220]/20'
                      : 'text-white/65 hover:bg-white/[0.07] hover:text-white'
                  }`
                }
              >
